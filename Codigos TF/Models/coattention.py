...
class Transformer(Module):
    def __init__(self,
        ...
        self.output_linear = torch.nn.Linear(64, d_output)
        
        #co atencion

        self.channel_projection = nn.Linear(d_model * d_channel , l)
        self.input_projection = nn.Linear(d_model * d_input , l)
        self.hz_projection = nn.Linear(d_model * d_hz , l)

        # Multi-Modal Factorized Bilinear Pooling (MFB) parameters
        #channel
        self.U = nn.Parameter(torch.randn(l, d_input * d_hz, f))
        #input
        self.V = nn.Parameter(torch.randn(l,d_channel * d_hz, f))
        #hz
        self.W = nn.Parameter(torch.randn(l,d_input * d_channel, f))

        # Attention map layers for image, question, and response
        self.image_attention = nn.Linear(l, g)
        self.question_attention = nn.Linear(l, g)
        self.response_attention = nn.Linear(l, g)

        # Attention weight parameters
        self.wmv = nn.Parameter(torch.randn(g, d_input * d_hz))
        self.wmq = nn.Parameter(torch.randn(g, d_channel * d_hz))
        self.wmr = nn.Parameter(torch.randn(g, d_input * d_channel))
    ...
    def forward(self, x, stage):
        ...
        encoding_1 = self.input_projection(encoding_1)

        encoding_2 = self.channel_projection(encoding_2)

        encoding_3 = self.hz_projection(encoding_3)

        z = encoding_1 * encoding_2 * encoding_3

        output = self.output_linear(z)
